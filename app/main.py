...
def nl_query(req: dict):
    try:
        query = req.get("query", "")
        start, end, fields = parse_nl_query(query)
        all_points = []

        for field in fields:
            flux = build_flux(start, end, SENSOR_TOPIC_MAP[field], field)
            payload = {
                "queries": [{
                    "refId": "A",
        ...
        return {"query": query, "summary": summary, "sample_points": all_points}
